...
public final class Dictionary
{
  ...
  static void loadMeta() throws IOException
  {
    ...
    Dictionary.inMeta =
        CompressHelper.getDecompressedInputStream(Dictionary.DXZ, Dictionary.HEADER_OFFSET, Dictionary.headerLen);
    ...
    Dictionary.inIndex =
        CompressHelper.getDecompressedInputStream(Dictionary.DXZ, Dictionary.idxOffset, Dictionary.idxLen);
    Dictionary.inData =
        CompressHelper.getDecompressedInputStream(Dictionary.DXZ, Dictionary.dataOffset, Dictionary.dataLen);
  }
  ...
  public static void loadCachedIndexes() throws IOException
  {
    ...
    try (InputStream inCached =
        CompressHelper.getDecompressedInputStream(Dictionary.DXZ, Dictionary.cachedOffset, Dictionary.cachedLen);)
    {
  ...
  public static void findExactAndStartsWithIndexes(IndexKey inputKey, final int idx)
  {
    try
    {
      ...
      while (i < Dictionary.totalDefs)
      {
        ...
        {
          // starts with
          if (inputKey.keyLength > MIN_INDEXES_STARTSWITH_KEYLENGTH)
          {
            Context.searchResult.idxStartsWiths.add(i);
          } else
          {
            break;
          }
   ...
   * @param inputKey
   * @param idx
   * @return 0: key bytes are the same, MAX_VALUE: src key starts with input
   *         key, <0: before, >0: after
   * @throws IOException
   */
  private static final int compare(IndexKey inputKey, int idx) throws IOException
  {
    ...
    // inputKey.keyLength));
    return ClientHelper.compareExtended(Dictionary.CACHE_INDEX, Dictionary.getCachedIndexOffset(idx)
        + IndexKey.KEY_OFFSET, CacheKey.KEY_LENGTH, inputKey.key, 0, inputKey.keyLength);
  }
  ...
  private static void readDataOffsets(int idxIndex) throws IOException
  {
    if (idxIndex != 0)
    {
      ...
      Dictionary.DATA_OFFSETS[0] =
          ClientHelper.toInt(Dictionary.CACHE_INDEX, Dictionary.getCachedIndexOffset(idxIndex - 1)
              + IndexKey.KEY_OFFSET + CacheKey.KEY_LENGTH + 2);
    } else
    {
    ...
    final int dataOffNext =
        ClientHelper.toInt(Dictionary.CACHE_INDEX, Dictionary.getCachedIndexOffset(idxIndex) + IndexKey.KEY_OFFSET
            + CacheKey.KEY_LENGTH + 2);
    Dictionary.DATA_OFFSETS[2] =
        ClientHelper.toShort(Dictionary.CACHE_INDEX, Dictionary.getCachedIndexOffset(idxIndex) + IndexKey.KEY_OFFSET
            + CacheKey.KEY_LENGTH);
  ...
  {
    final boolean findContains = inputKey.keyLength > MIN_INDEXES_CONTAINS_KEYLENGTH;
    if (findContains)
    {
      try
      {
        ...
        while (i < Dictionary.totalDefs)
        {
          ...
          {
            // ends with
            Context.searchResult.idxEndsWiths.add(i);
          } else if (cmp >= 0)
          {
            // contains
        ...
        System.out.println("total: " + totalDefs + ", i: " + i);
      } catch (IOException e)
      {
        // TODO Auto-generated catch block
        e.printStackTrace();
  ...
  private static final int contains(IndexKey inputKey, int idx) throws IOException
  {
    ...
    return ClientHelper.containsExtended(Dictionary.CACHE_INDEX, Dictionary.getCachedIndexOffset(idx)
        + IndexKey.KEY_OFFSET, CacheKey.KEY_LENGTH, inputKey.key, 0, inputKey.keyLength);
