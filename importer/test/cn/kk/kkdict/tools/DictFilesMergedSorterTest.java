...
public class DictFilesMergedSorterTest {
    ...
    public void testSortWriteSkipped() {
        DictFilesMergedSorter sorter = new DictFilesMergedSorter(Language.ZH, TMP_DIR, "result.txt", false, true,
        ...
        try {
            ...
            assertEquals(
                    ...
                    ArrayHelper.toString(Helper.readBytes(TMP_DIR
                            + File.separator
                            + Helper.appendFileName(new File("result.txt").getName(),
                                    DictFilesMergedSorter.SUFFIX_SKIPPED))));
        } catch (Throwable t) {
            t.printStackTrace();
            fail(t.toString());
        }
    ...
    public void testSortWithoutSkipped() {
        DictFilesMergedSorter sorter = new DictFilesMergedSorter(Language.ZH, TMP_DIR, "result.txt", true, false,
                testFile1, testFile2, testFile3);
        sorter.setFilterAttributes(false);
    ...
    public void testSortWithSkipped() {
        DictFilesMergedSorter sorter = new DictFilesMergedSorter(Language.ZH, TMP_DIR, "result.txt", false, false,
                testFile1, testFile2, testFile3);
        sorter.setFilterAttributes(false);
