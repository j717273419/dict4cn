...
public class WikiConfluencer {
    ...
    private final static String substringBetweenLast(String text, String start, String end) {
    ...
    private static boolean isNotEmptyOrNull(String text) {
        return text != null && text.length() > 0;
    }

    private static void precheck() {
        if (!new File(HAN_DE_DICT_UTF8_FILE).isFile()) {
            System.err.println("Could not read input file: " + HAN_DE_DICT_UTF8_FILE);
            System.exit(-100);
        }
        if (!(new File(OUT_DIR).isDirectory() || new File(OUT_DIR).mkdirs())) {
            System.err.println("Could not create output directory: " + OUT_DIR);
            System.exit(-101);
        }
    }
    ...
    public static void main(String args[]) throws IOException, SQLException {
        ...
        BufferedWriter skippedIncompleteWriter = new BufferedWriter(new FileWriter(OUT_DIR + File.separator
                + "skipped-incomplete.txt"), 8192000);

        String line;
        String name = null;
        String pinyin;
        String translation;
        Set<String> globalCategories = new TreeSet<String>();
        ...
        while ((line = reader.readLine()) != null) {
            if ((tmp = substringBetween(line, " ", " ")) != null) {
                name = tmp;
                pinyin = substringBetween(line, "[", "]");
                translation = substringBetween(line, "/", "/");
                if (isNotEmptyOrNull(name) && isNotEmptyOrNull(pinyin) && isNotEmptyOrNull(translation)
                        && !translation.contains("???")) {
                    categories = new TreeSet<String>();
                    descriptions = new TreeSet<String>();
        ...
        System.out.println("\n==============\nExtract HanDeDict Duration: "
                + formatDuration(System.currentTimeMillis() - timeStarted));
