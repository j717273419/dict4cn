...
public class DictByteBufferRow {
    ...
    private ArrayList<IntList> attrStartIdx = new ArrayList<IntList>(INITIAL_DEFS_SIZE);
    private ArrayList<IntList> attrStopIdx = new ArrayList<IntList>(INITIAL_DEFS_SIZE);
    ...
    public void debug(final int i) {
        System.out.println("DictByteBufferRow [size=" + size + ", attrsAnalyzed=" + attrsAnalyzed + ", attrsSize="
                + attrsSize.get(i) + ", attrsStartIdx=" + attrsStartIdx.get(i) + ", attrsStopIdx="
                + attrsStopIdx.get(i) + ", defStartIdx=" + defStartIdx.get(i) + ", defStopIdx=" + defStopIdx.get(i)
                + ", lngStartIdx=" + lngStartIdx.get(i) + ", lngStopIdx=" + lngStopIdx.get(i) + ", valueStartIdx="
                + valueStartIdx.get(i) + ", valueStopIdx=" + valueStopIdx.get(i) + "]");
    }
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (idx == -1) {
                ...
                if (bb.hasRemaining() && other.bb.hasRemaining() && ArrayHelper.equalsP(bb, other.bb)) {
                    ...
                    // System.out.println(ArrayHelper.toStringP(bb) + ", " + ArrayHelper.toStringP(other.bb));
                    return false;
                }
            }
        }
    ...
    public final ByteBuffer getValue(final int i) {
        if (i < size) {
            // System.out.println("start: " + valueStartIdx.get(i) + ", stop:" + valueStopIdx.get(i));
    ...
    public boolean isLinkedBy(final DictByteBufferRow other) {
        if (!isEmpty() && !other.isEmpty()) {
            ...
            for (int i = 0; i < other.size; i++) {
                idx = indexOfLanguage(other.getLanguage(i));
                if (idx != -1 && ArrayHelper.equalsP(getValue(idx), other.getValue(i))) {
                    return true;
                }
            }
        }
                ...
                if (!sepDefFound) {
                    if (b == Helper.SEP_DEFINITION_BYTES[sepDefIdx]) {
                        if (++sepDefIdx >= Helper.SEP_DEFINITION_BYTES.length) {
                            // value start
                            lngStopIdx.set(idx, i - Helper.SEP_DEFINITION_BYTES.length + 1);
                    ...
                    if (!sepAttrFound) {
                        if (b == Helper.SEP_ATTRS_BYTES[sepAttrIdx]) {
                            if (++sepAttrIdx >= Helper.SEP_ATTRS_BYTES.length) {
                                // attrs start
                                valueStopIdx.set(idx, i - Helper.SEP_ATTRS_BYTES.length + 1);
                ...
                for (int i = start; i < end; i++) {
                    ...
                    if (b == Helper.SEP_ATTRS_BYTES[sepIdx]) {
                        if (++sepIdx >= Helper.SEP_ATTRS_BYTES.length) {
                            ...
                            currentAttrsStop.set(attrIdx, i - Helper.SEP_ATTRS_BYTES.length + 1);
                            attrIdx++;
