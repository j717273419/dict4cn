...
CREATE PROCEDURE `dict2go`.`addTranslation` (
 srckey BINARY(50), srclng INT UNSIGNED, tgtlng INT UNSIGNED, srcval VARCHAR(1000),
...
BEGIN
  ...
  IF (old_trlid > 0) THEN
    IF (NOT old_tgtval LIKE '%{$tgtval}%') THEN
      IF (tgtval LIKE '%{$old_tgtval}%') THEN
